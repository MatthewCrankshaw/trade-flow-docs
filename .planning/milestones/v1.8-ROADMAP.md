...
- **41, 43 → 44:** Entity + UI required before Send dialog exists. Phase 44 also introduces the new standalone `estimate-settings` module and updates the Business > Templates tab in trade-flow-ui to fetch/save estimate and quote templates via two independent APIs.
...
**Success Criteria** (what must be TRUE):
  ...
  2. The Business > Templates tab in trade-flow-ui is updated to fetch and save both template types in parallel via two independent APIs (`quote-settings` and `estimate-settings`), and the existing quote template UI/UX still loads and saves unchanged.
