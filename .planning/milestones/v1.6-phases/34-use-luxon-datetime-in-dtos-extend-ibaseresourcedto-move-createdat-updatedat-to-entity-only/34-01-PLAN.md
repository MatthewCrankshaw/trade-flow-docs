...
must_haves:
  truths:
    - "Shared toDateTime and toOptionalDateTime utility exists in src/core/utilities/ with unit tests"
    - "ISubscriptionDto extends IBaseResourceDto and has no createdAt or updatedAt fields"
    - "All date fields on ISubscriptionDto are Luxon DateTime (currentPeriodEnd, trialEnd, canceledAt)"
    - "No DTO across the API uses native JS Date for date fields — all use Luxon DateTime"
    - "Date-to-DateTime conversion happens in repository layer via shared utility"
    - "API CLAUDE.md documents Luxon DateTime and ISO 8601 UTC standards"
  artifacts:
    - "trade-flow-api/src/core/utilities/to-date-time.utility.ts"
