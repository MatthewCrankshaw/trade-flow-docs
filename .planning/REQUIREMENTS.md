...
- [ ] **BILL-01**: `GET /v1/subscription` returns the current user's subscription status, `currentPeriodEnd`, `trialEnd`, and `cancelAtPeriodEnd`
- [ ] **BILL-02**: `DELETE /v1/subscription` cancels the subscription at period end (`cancel_at_period_end: true`) — access continues until `currentPeriodEnd`
- [ ] **BILL-03**: `POST /v1/subscription/portal` creates a Stripe Billing Portal session and returns the portal URL for redirect
