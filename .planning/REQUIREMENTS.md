...
- [ ] **SND-02**: A new `estimate-settings` module exposes `estimateEmailTemplate` via its own API; the Business > Templates tab in trade-flow-ui fetches and saves estimate and quote templates via their two independent APIs (`quote-settings` is untouched)
