...
- **Integration test for concurrent revise (SC #4):** planner designs this. Minimum: two parallel `EstimateReviser.revise` calls on the same source id, assert exactly one returns successfully and one throws `ConflictError`, assert the `estimates` collection has exactly one row with `isCurrent: true` in the chain. Mongo in-memory server is fine (existing pattern).
